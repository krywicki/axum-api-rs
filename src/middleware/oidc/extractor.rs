...
use axum::{
    ...
    http::{self, uri::PathAndQuery, Uri},
    response::{IntoResponse, Redirect},
};
use openidconnect::{core::CoreGenderClaim, IdTokenClaims};

use super::{
    error::{self, ExtractorError},
    oidc::{AdditionalClaims, ClearSessionFlag},
};
...
where
    ...
    AC: AdditionalClaims,
{
    type Rejection = error::ExtractorError;
    ...
    async fn from_request_parts(
        ...
        parts
            ...
            .get::<Self>()
            .cloned()
            .ok_or(error::ExtractorError::Unauthorized)
    }
}
...
pub struct OidcRpInitiatedLogout {
    pub end_session_endpoint: Uri,
    pub id_token_hint: String,
    pub client_id: String,
    pub post_logout_redirect_uri: Option<Uri>,
...
impl OidcRpInitiatedLogout {
    ...
    pub fn with_post_lougout_redirect(mut self, uri: Uri) -> Self {
    ...
    pub fn uri(&self) -> Result<Uri, http::Error> {
        ...
        let path_and_query = match parts.path_and_query {
            Some(path_and_query) => {
                PathAndQuery::from_maybe_shared(format!("{}?{query}", path_and_query.path()))
            }
    ...
    S: Send + Sync,
{
    ...
    async fn from_request_parts(
        ...
        {
            Some(this) => Ok(this),
            None => Err(error::ExtractorError::RpInitiatedLogoutNotSupported),
        }
    }
}
...
impl IntoResponse for OidcRpInitiatedLogout {
    fn into_response(self) -> axum::response::Response {
        if let Ok(uri) = self.uri() {
            ...
            error::ExtractorError::FailedToCreateRpInitiatedLogoutUri.into_response()
        }
    }
}
